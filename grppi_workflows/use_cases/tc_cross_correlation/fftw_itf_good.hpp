...
class fftw_itf {
  ...
  private:
    ...
    std::map<std::pair<int,int>, fftw_complex *> in_;
    std::map<std::pair<int,int>, fftw_complex *> out_;
                ...
                int plan_sign, unsigned int flags)
    {
        ...
        // if size has change, calculate from scratch but thread safe
        std::unique_lock<std::mutex> lock(fftw_itf::Singleton().mutex_);
        if (fftw_itf::Singleton().plans_.find(std::make_pair(in.size(),plan_sign)) ==
                                                        fftw_itf::Singleton().plans_.end()) {
            fftw_itf::Singleton().cond_plan_.wait(lock,
                            [] {return !fftw_itf::Singleton().planning_;});
            fftw_itf::Singleton().planning_ = true;
            if (fftw_itf::Singleton().plans_.find(std::make_pair(in.size(),plan_sign)) ==
                                                        fftw_itf::Singleton().plans_.end()) {
                fftw_itf::Singleton().in_[std::make_pair(in.size(),plan_sign)] =
                                    (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * in.size());
                fftw_itf::Singleton().out_[std::make_pair(in.size(),plan_sign)] =
                                    (fftw_complex*) fftw_malloc(sizeof(fftw_complex) * in.size());
                fftw_itf::Singleton().plans_[std::make_pair(in.size(),plan_sign)] =
                        fftw_plan_dft_1d(in.size(),
                                    fftw_itf::Singleton().in_[std::make_pair(in.size(),plan_sign)],
                                    fftw_itf::Singleton().out_[std::make_pair(in.size(),plan_sign)],
                                    plan_sign, flags);
            ...
            fftw_itf::Singleton().planning_ = false;
            fftw_itf::Singleton().cond_plan_.notify_all();
        }
        fftw_plan actual_plan = fftw_itf::Singleton().plans_[std::make_pair(in.size(),plan_sign)];
        fftw_complex * aux_in = fftw_itf::Singleton().in_[std::make_pair(in.size(),plan_sign)];
        fftw_complex * aux_out = fftw_itf::Singleton().out_[std::make_pair(in.size(),plan_sign)];
        
        lock.unlock();
 
        memcpy (aux_in, in.data(), sizeof(std::complex<double>)*in.size());

        //std::cout <<  "Plan execute: "  << std::endl;
