...
class Product(GenericPE):
    ...
    def process(self, inputs):
        store = []
        dirs = [x for x in os.listdir(ROOT_DIR + 'DATA/'+ starttime) if not x.startswith('.')]
        for dir in dirs:
            files = [x for x in os.listdir(ROOT_DIR+'/DATA/'+ starttime +'/'+ dir) if not x.startswith('.')]
            for f in files:
...
class Plot(ConsumerPE):
    ...
    def _process(self, data):
        xcorr1 = data[2]
        fout = ROOT_DIR+'XCORR/%s_%s_%s.plot.eps' % (self.filename, data[0], data[1])
        sps=4
